...
int main() {
	...
	int i = 0, n = 0, size = -1, k = -1, p = -1, q = -1, opt = -1, count = 0;
	...
	while (cycle) {
		...
		case 1:
			if (isActive) {
				...
				while (true) {
					cout << "Unesite parametar p primarne hes funkcije, velicinu baketa i parametre p i q sekundarne hes funkcije odvojene razmakom: ";
					cin >> size >> k >> p >> q;
					if (size < 1 || k < 1 || p < 1 || q < 0) cout << "Neispravne vrednosti parametara!" << endl;
					else {
						h.set(size, k, p, q);
						isActive = true;
						break;
